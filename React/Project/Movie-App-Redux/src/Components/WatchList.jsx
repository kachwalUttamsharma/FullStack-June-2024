...
const WatchList = () => {
  ...
  const { watchList, genreList, currGenre, search } = useSelector(
    (state) => state.WatchListSlice
  );

  useEffect(() => {
    const genreSet = new Set(
      watchList.map((movie) => genreids[movie.genre_ids[0]] || "Unknown")
    );
    dispatch(setGenreList(["All Genre", ...Array.from(genreSet)]));
  }, [watchList, dispatch]);

  const handleAscendingOrderRatings = () => {
    dispatch(sortWatchListByRating("asc"));
  };

  const handleDescendingOrderRatings = () => {
    dispatch(sortWatchListByRating("desc"));
  };
